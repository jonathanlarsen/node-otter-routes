...
exports = module.exports = function ModuleManagement(app, options, next) {
    router.get('/modules/remove/:module', function(req, res) {
        ...
        if (app._otterRoutes) {
            if (app._otterRoutes[moduleName] && app._otterRoutes[moduleName].requireCacheKeys) {
                app._otterRoutes[moduleName].requireCacheKeys.forEach(function(file) {
                    myrepl.context.require.uncache(file);
                });
            }
            delete app._otterRoutes[moduleName];
        }
    ...
    router.get('/modules/add/:module', function(req, res) {
        ...
        require('./otter')(app, options.directory, options, function() {
            res.json({message: moduleName + ' added'});
        });
    });
